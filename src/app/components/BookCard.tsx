...
const BookCard: React.FC<BookCardProps> = ({ book, onAddToCart }) => {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300">
      ...
      <div className="p-4">
        <Link href={`/book/${book.id}`} className="block cursor-pointer">
          <h3 className="text-lg font-semibold text-gray-800 truncate hover:text-green-600 transition-colors duration-200">{book.title}</h3>
        ...
        <div className="flex items-center justify-between mt-3">
          ...
          {!book.inStock && (
            <span className="text-xs font-bold text-white bg-red-500 px-2 py-1 rounded-md">Out of Stock</span>
          )}
        </div>
